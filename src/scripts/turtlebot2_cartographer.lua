...
options = {
  ...
  published_frame = "odom",         -- Cartographer publishes map->odom; robot TF handles odom->base_footprint
  odom_frame = "odom",
  provide_odom_frame = false,       -- use existing /odom from kobuki
  publish_frame_projected_to_2d = false,
  use_odometry = true,              -- throttled to 10 Hz via /odom_throttled (avoids duplicate timestamp crash)
  use_nav_sat = false,
  use_landmarks = false,
